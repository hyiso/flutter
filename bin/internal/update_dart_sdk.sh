...
ENGINE_VERSION=$(cat "$FLUTTER_ROOT/bin/internal/engine.ohos.version")
...
if [ ! -f "$ENGINE_STAMP" ] || [ "$ENGINE_VERSION" != `cat "$ENGINE_STAMP"` ]; then
  ...
  DART_SDK_BASE_URL="${FLUTTER_OHOS_STORAGE_BASE_URL:-https://yfd-tool.oss-cn-beijing-internal.aliyuncs.com}${ENGINE_REALM:+/$ENGINE_REALM}"

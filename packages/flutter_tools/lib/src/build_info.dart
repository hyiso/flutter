...
class BuildInfo {
  ...
  /// Convert this config to a series of project level arguments to be passed
  /// on the command line to hvigor.
  List<String> toHvigorConfig() {
    // PACKAGE_CONFIG not currently supported.
    final List<String> result = <String>[
      if (dartDefines.isNotEmpty)
        ...<String>['-p', 'dartDefines=${encodeDartDefines(dartDefines)}'],
      ...<String>['-p', 'dartObfuscation=$dartObfuscation'],
      if (extraFrontEndOptions.isNotEmpty)
        ...<String>['-p', 'extraFrontEndOptions=${extraFrontEndOptions.join(',')}'],
      if (extraGenSnapshotOptions.isNotEmpty)
        ...<String>['-p', 'extraGenSnapshotOptions=${extraGenSnapshotOptions.join(',')}'],
      if (splitDebugInfoPath != null)
        ...<String>['-p', 'splitDebugInfo=$splitDebugInfoPath'],
      ...<String>['-p', 'trackWidgetCreation=$trackWidgetCreation'],
      ...<String>['-p', 'treeShakeIcons=$treeShakeIcons'],
      if (performanceMeasurementFile != null)
        ...<String>['-p', 'performanceMeasurementFile=$performanceMeasurementFile'],
      if (bundleSkSLPath != null)
        ...<String>['-p', 'bundleSkSLPath=$bundleSkSLPath'],
      if (codeSizeDirectory != null)
        ...<String>['-p', 'codeSizeDirectory=$codeSizeDirectory'],
    ];
    return result;
  }
}
...
String? validatedBuildNumberForPlatform(TargetPlatform targetPlatform, String? buildNumber, Logger logger) {
  ...
  if (targetPlatform == TargetPlatform.ohos_arm64 ||

...
class PlatformViewsService {
  ...
  /// {@template flutter.services.PlatformViewsService.initOhosView}
  /// Creates a controller for a new Ohos view.
  ///
  /// The `id` argument is an unused unique identifier generated with
  /// [platformViewsRegistry].
  ///
  /// The `viewType` argument is the identifier of the Ohos view type to be
  /// created, a factory for this view type must have been registered on the
  /// platform side. Platform view factories are typically registered by plugin
  /// code. Plugins can register a platform view factory with
  /// [PlatformViewRegistry#registerViewFactory](/javadoc/io/flutter/plugin/platform/PlatformViewRegistry.html#registerViewFactory-java.lang.String-io.flutter.plugin.platform.PlatformViewFactory-).
  ///
  /// The `creationParams` argument will be passed as the args argument of
  /// [PlatformViewFactory#create](/javadoc/io/flutter/plugin/platform/PlatformViewFactory.html#create-ohos.content.Context-int-java.lang.Object-)
  ///
  /// The `creationParamsCodec` argument is the codec used to encode
  /// `creationParams` before sending it to the platform side. It should match
  /// the codec passed to the constructor of
  ...
  /// [StringCodec], or [BinaryCodec].
  ///
  /// The `onFocus` argument is a callback that will be invoked when the Ohos
  /// View asks to get the input focus.
  ///
  /// The Ohos view will only be created after
  /// [OhosViewController.setSize] is called for the first time.
  ///
  /// If `creationParams` is non null then `creationParamsCodec` must not be
  /// null.
  /// {@endtemplate}
  ...
  /// {@macro flutter.services.PlatformViewsService.initOhosView}
  ///
  /// This attempts to use the newest and most efficient platform view
  /// implementation when possible. In cases where that is not supported, it
  /// falls back to using Hybrid Composition, which is the mode used by
  /// [initExpensiveOhosView].
  ...
  /// {@macro flutter.services.PlatformViewsService.initOhosView}
  ///
  /// When this factory is used, the Ohos view and Flutter widgets are
  /// composed at the Ohos view hierarchy level.
  ///
  /// Using this method has a performance cost on devices running Ohos 9 or
  /// earlier, or on underpowered devices. In most situations, you should use
  /// [initOhosView] or [initSurfaceOhosView] instead.
...
/// Properties of an Ohos Touch.
///
/// A Dart version of Ohos's [Touch](https://developer.huawei.com/consumer/cn/doc/harmonyos-references-V5/js-apis-touchevent-V5#touch).
class OhosTouch {
  /// Creates an [OhosTouch] object.
  ///
  ...
  /// Value for `toolType` when the tool type is unknown.
  static const int kToolTypeUnknown = -1;

  /// Value for `toolType` when the tool type is a finger.
  static const int kToolTypeFinger = 0;

  /// Value for `toolType` when the tool type is a stylus (pen).
  static const int kToolTypeStylus = 1;

  /// Value for `toolType` when the tool type is a rubber.
  static const int kToolTypeRubber = 2;

  /// Value for `toolType` when the tool type is a brush.
  static const int kToolTypeBrush = 3;

  /// Value for `toolType` when the tool type is a pencil.
  static const int kToolTypePencil = 4;

  /// Value for `toolType` when the tool type is an airbrush.
  static const int kToolTypeAirBrush = 5;

  /// Value for `toolType` when the tool type is a mouse.
  static const int kToolTypeMouse = 6;

  /// Value for `toolType` when the tool type is an lens.
  static const int kToolTypeLens = 7;
...
/// Coordinates for an Ohos pointer.
///
/// A Dart version of Ohos's [PointerCoords](https://developer.huawei.com/consumer/cn/doc/harmonyos-references-V5/js-apis-pointer-V5).
...
/// An OhosTouchEvent.
///
/// A Dart version of Ohos's [MotionEvent](https://developer.huawei.com/consumer/cn/doc/harmonyos-references-V5/js-apis-touchevent-V5).
class OhosMotionEvent {
  ...
  /// List of [OhosTouch] for each pointer that is part of this event.
  final List<OhosTouch> pointerProperties;
  ...
  List<dynamic> _asList(int viewId) {
    return <dynamic>[
      ...
      action,
      pointerCount,
      pointerProperties.map<List<int>>((OhosTouch p) => p._asList()).toList(),
...
// Helper for converting PointerEvents into OhosMotionEvents.
class _OhosMotionEventConverter {
  ...
  final Map<int, OhosTouch> pointerProperties =
      <int, OhosTouch>{};
  ...
  OhosMotionEvent? toOhosMotionEvent(PointerEvent event) {
    ...
    return OhosMotionEvent(
      ...
      pointerCount: pointerPositions.length,
      pointerProperties: pointers
          .map<OhosTouch>((int i) => pointerProperties[i]!)
          .toList(),
      pointerCoords: pointers
      ...
      deviceId: 0,
      edgeFlags: 0,
      source: _OhosMotionEventConverter.sourceFor(event),
      flags: 0,
      motionEventId: event.embedderId,
    );
  }
  ...
  OhosTouch propertiesFor(PointerEvent event, int pointerId) {
    return OhosTouch(id: pointerId, toolType: switch (event.kind) {
      PointerDeviceKind.touch          => OhosTouch.kToolTypeFinger,
      PointerDeviceKind.trackpad       => OhosTouch.kToolTypeFinger,
      PointerDeviceKind.mouse          => OhosTouch.kToolTypeMouse,
      PointerDeviceKind.stylus         => OhosTouch.kToolTypeStylus,
      PointerDeviceKind.invertedStylus => OhosTouch.kToolTypeStylus,
      PointerDeviceKind.unknown        => OhosTouch.kToolTypeUnknown,
    });
  }
...
/// An Ohos view controller.
///
/// This controller is used to create and interact with the underlying Ohos view.
abstract class OhosViewController extends PlatformViewController {
  ...
  /// Ohos' [TextDirection.rtl] (https://developer.huawei.com/consumer/cn/doc/harmonyos-references-V5/js-apis-graphics-text-V5#textdirection)
  static const int kOhosTextDirectionRtl = 0;

  /// Ohos' [TextDirection.ltr] (https://developer.huawei.com/consumer/cn/doc/harmonyos-references-V5/js-apis-graphics-text-V5#textdirection)
  static const int kOhosTextDirectionLtr = 1;
  ...
  static int _getOhosDirection(TextDirection direction) {
    return switch (direction) {
      TextDirection.ltr => kOhosTextDirectionLtr,
      TextDirection.rtl => kOhosTextDirectionRtl,
    };
  }
...
/// Controls an Ohos view that is composed using a GL texture.
/// This controller is created from the [PlatformViewsService.initSurfaceOhosView] factory,
/// and is defined for backward compatibility.
...
/// Controls an Ohos view that is composed using a hybrid composition.
///
/// This controller is created from the [PlatformViewsService.initHybridOhosView] factory.
///
/// This controller is used to create and interact with the underlying Ohos view.
...
class _TextureOhosViewControllerInternals extends _OhosViewControllerInternals {
  ...
  Future<Size> setSize(
    ...
    assert(meta!.containsKey('width'));
    assert(meta!.containsKey('height'));
    final int width = meta!['width']! as int;
    final int height = meta['height']! as int;
